...
class TreeParser:
            ...
            if self.tokens[i] == Token.REF:
                ...
                if isinstance(next_token, tuple) and next_token[0] == Token.PROP:
                    if next_token[1] not in defaults:
                        raise BadExpression(
                            f"Trying to access @{next_token[1]}, but it is not present in the dictionary")
    ...
    def parse(self, 
              ...
              defaults: Optional[Dict[str, Union[str, List[str]]]] = None) -> Union[ET.Element, List[ET.Element]]:
        '''
        ...
        Operators:
            ...
            {Token.OPENBR}A{Token.SEP} V {Token.OR} A{Token.SEP} V {Token.AND} V {Token.AND} {Token.REF}A{Token.CLOSEBR} 
            -- A conditional predicate that filters tags where
                A  -- attribute name
        ...
        if self.tokens[-1] != Token.TERM:
            raise BadExpression("Expression must end with ';'")
        ...
        while i != len(self.tokens):
            ...
            if isinstance(token, tuple):
                ...
                if token == Token.WILDCARD:
                    ...
                    self.__insert_wildcard(wildcards, t_value)
                ...
                # Find all instances (MANOIDS SON!)
                current_tag = sum([f for f in map(lambda x: x.getparent().findall(x.tag), current_tag)], [])
